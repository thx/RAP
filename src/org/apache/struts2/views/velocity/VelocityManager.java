/*
 * $Id: VelocityManager.java,v 1.1 2010/01/11 12:57:32 zhangchengsong Exp $
 *
...
public class VelocityManager {
	 ...
	 */
	/*
	 * public synchronized static VelocityManager getInstance() { if (instance
	 * == null) { String classname = VelocityManager.class.getName();
	 * 
	 * if (Settings.isSet(StrutsConstants.STRUTS_VELOCITY_MANAGER_CLASSNAME)) {
	 * classname =
	 * Settings.get(StrutsConstants.STRUTS_VELOCITY_MANAGER_CLASSNAME).trim(); }
	 * 
	 * if (!classname.equals(VelocityManager.class.getName())) { try {
	 * log.info("Instantiating VelocityManager!, " + classname); // singleton
	 * instances shouldn't be built accessing request or session-specific
	 * context data instance = (VelocityManager)
	 * ObjectFactory.getObjectFactory().buildBean(classname, null); } catch
	 * (Exception e) { log.fatal(
	 * "Fatal exception occurred while trying to instantiate a VelocityManager instance, "
	 * + classname, e); instance = new VelocityManager(); } } else { instance =
	 * new VelocityManager(); } }
	 * 
	 * return instance; }
	 */
	 ...
	 * <li><strong>struts</strong> - an instance of
	 * {@link org.apache.struts2.util.StrutsUtil}</li>
	...
	public Context createContext(ValueStack stack, HttpServletRequest req,
		...
		if (toolboxManager != null && ctx != null) {
			...
			ChainedContext chained = new ChainedContext(getVelocityEngine(),
					req, res, ctx);
	...
	protected VelocityContext[] prepareChainedContexts(
		...
		for (int i = 0; i < chainedContextNames.length; i++) {
			...
			try {
				VelocityContext velocityContext = (VelocityContext) objectFactory
						.buildBean(className, null);
				contextList.add(velocityContext);
			} catch (Exception e) {
				log.warn("Warning.  "
						+ e.getClass().getName()
						+ " caught while attempting to instantiate a chained VelocityContext, "
						+ className + " -- skipping");
			}
		}
		if (contextList.size() > 0) {
			VelocityContext[] extraContexts = new VelocityContext[contextList
					.size()];
		...
		try {
			if (context.getRealPath(configfile) != null) {
				...
				if (filename != null) {
					...
					if (file.isFile()) {
						resourceLocation = file.getCanonicalPath()
								+ " from file system";
					...
					if (in == null) {
						file = new File(context.getRealPath("/WEB-INF/"
								+ configfile));
			...
			// our classpath
			if (in == null) {
				in = VelocityManager.class.getClassLoader()
			...
			log.warn("Unable to load velocity configuration "
					+ resourceLocation, e);
		} finally {
			if (in != null) {
				try {
	...
	/**
	 * allow users to specify via the struts.properties file a set of additional
	 * VelocityContexts to chain to the the StrutsVelocityContext. The intent is
	 * to allow these contexts to store helper objects that the ui developer may
	...
	protected void initToolbox(ServletContext context) {
		...
		if (toolBoxLocation != null) {
			toolboxManager = ServletToolboxManager.getInstance(context,
					toolBoxLocation);
		} else {
	...
	private void applyDefaultConfiguration(ServletContext context, Properties p) {
		...
		if (context.getRealPath("") != null) {
			p.setProperty("strutsfile.resource.loader.description",
					"Velocity File Resource Loader");
			p.setProperty("strutsfile.resource.loader.class",
					"org.apache.velocity.runtime.resource.loader.FileResourceLoader");
			p.setProperty("strutsfile.resource.loader.path",
					context.getRealPath(""));
			p.setProperty(
					"strutsfile.resource.loader.modificationCheckInterval", "2");
		 ...
		 */
		p.setProperty("strutsclass.resource.loader.description",
				"Velocity Classpath Resource Loader");
		p.setProperty("strutsclass.resource.loader.class",
				"org.apache.struts2.views.velocity.StrutsResourceLoader");
		p.setProperty("strutsclass.resource.loader.modificationCheckInterval",
				"2");
	...
	private static final String replace(String string, String oldString,
			String newString) {
		if (string == null) {
			return null;
		}
